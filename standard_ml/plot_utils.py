...
def visualize_precision_recall_with_variations(list_of_Y_test, results_dict,
    ...
    plt.title('2-class Precision-Recall curve: AP={0:0.2f}'.format(avg_weighted_precision),
             fontsize=30)
...
def visualize_feature_outcome_pd(impact_outcome_pd,
    ...
    ax = fig.gca()
    impact_outcome_pd.sort_values(by=score_name,
                                  ascending=False).iloc[:10,:].plot(x=feature_name,
                           y=score_name,
...
def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None,
    ...
    Source: # Adapted from SKLearn example website
